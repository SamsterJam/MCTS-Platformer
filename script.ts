...
class Game {
  ...
  calculateBestPath(): Array<{x: number, y: number}> {
    ...
    for (let i = 0; i < MAX_DEPTH; i++) {
      ...
      const result = this.stepGame(currentState, bestAction);
  ...
  renderGameplay(): void {
    ...
    if (this.bestPath.length > 1) {
      ...
      this.ctx.beginPath();
      this.ctx.moveTo(
        this.bestPath[0].x * GRID_SIZE + GRID_SIZE/2, 
        this.bestPath[0].y * GRID_SIZE + GRID_SIZE/2
      );
